...
tocq_words <- tocq %>%
  ...
  anti_join(stop_words) ##removing stop words from dataset
tocq_dtm <- tocq_words %>%
  cast_dtm(booknumber, word, n) #cast_dtm creates a DTM (table where rows represent books and )
...
tocq_top_terms %>%
  ...
  scale_y_reordered() +
  theme_tufte(base_family = "Helvetica") ##adjusts the appearance of graph with Tufte's principles of effective data visualization, base_family means font
...
keyhar_topics
```
#wife has the highest prob of appearing/belonging to topic 4 (e-02 and the highest number)
We now have data stored as one topic-per-term-per-row. The betas listed here represent the probability that the given term belongs to a given topic. So, here, we see that the term "democratic" is most likely to belong to topic 4. Strictly, this probability represents the probability that the term is generated from the topic in question.
We can then plots the top terms, in terms of beta, for each topic as follows:
```{r, echo=FALSE}
